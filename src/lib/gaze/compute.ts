...
export const PHI = (1 + SQ5) / 2;
export const K = (3 * SQ5 - 7) / 2;

export type LM = { x: number; y: number };

type EyeSpec = {
  pupil: number; corner1: number; corner2: number; upper: number; lower: number; irisRing: number[];
};
...
export type GazeResult = {
  ...
  ox: number; oy: number;
  // debug: per-eye iris centers so we can draw them
...
class Smoother {
  ...
  update(nx: number, ny: number): [number, number] {
    ...
    else { const a = this.alpha, b = 1 - a; this.vx = a*nx + b*this.vx; this.vy = a*ny + b*this.vy; }
...
const idx = (e: EyeSpec): EyeIdx => ({
  p: e.pupil<<1, c1: e.corner1<<1, c2: e.corner2<<1, u: e.upper<<1, l: e.lower<<1, ring: e.irisRing.map(i=>i<<1),
});
...
function irisCenterFromBuf(buf: Float32Array, pupil:number, ring:number[]): [number, number] {
  ...
  let sx=0, sy=0; for (const r of ring){ sx += buf[r]; sy += buf[r+1]; }
...
function normalizedEyeOffset(buf: Float32Array, eye:EyeIdx, tmp:Float32Array, phi=PHI){
  ...
  const xNorm = (ix - ecx)/scale, yNorm = (iy - ecy)/scale;
  const inv = 1 / (dome(xNorm, yNorm, PHI) || 1); // smooth radial scaling
...
function flattenLM(lm: LM[]): Float32Array {
  const need = lm.length<<1; bufferPool.ensure(need);
  const out = bufferPool.lmBuf!; for (let i=0,j=0;i<lm.length;i++){ out[j++]=lm[i].x; out[j++]=lm[i].y; }
...
export class GazeProjectionTS {
  ...
  computeTyped(buf: Float32Array, n: number): GazeResult | null {
    ...
    // Direction = mean of normalized offsets
    ...
    // ORIGIN = midpoint of iris centers
    ...
    return {
      ...
      ox, oy,
      lx: Loff.ix, ly: Loff.iy, rx: Roff.ix, ry: Roff.iy
    };
  }
  ...
  reset(){ this.smoother.reset(); bufferPool.reset(); this.tmpL[0]=this.tmpL[1]=this.tmpR[0]=this.tmpR[1]=0; }
